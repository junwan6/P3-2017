...
$(window).load(function(){
    ...
    $('#occupationPlotter').mousemove(function(event) {
        ...
        hoverRow = (career ? career.soc : null);
...
function render() {
    ...
    if (hoverTitle) {
        ...
        let careerRow = document.getElementById(hoverRow);
        careerRow.classList.add('careerRowHover');
    }
    else {
        $('#hoverOccupationTitle').hide();
        $('.careerRow').removeClass('careerRowHover');
    }

    if (hoverRow){
      let dot = new Object();
      $('career').each(function(index) {
        if ($(this).attr('soc') == hoverRow) {
          dot.title = $(this).attr('title');
          dot.x = $(this).attr('x');
          dot.y = $(this).attr('y');
        }
      });
      ...
      var pageOffsetX = dot.x * (width/2) + (width/2) + offset.left;
      var pageOffsetY = -(dot.y * (height/2) - (height/2) - offset.top);
      $('#hoverOccupationTitle').show();
      $('#hoverOccupationTitle').offset({top: pageOffsetY - 10, left: pageOffsetX + 20});
      $('#hoverOccupationTitle').text(dot.title);
